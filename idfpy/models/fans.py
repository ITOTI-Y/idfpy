...
class FanSystemModelSpeedFractionsItem(IDFBaseModel):
    ...
    speed_electric_power_fraction: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
        json_schema_extra={
...
class FanComponentModel(IDFBaseModel):
    ...
    maximum_flow_rate: float | Literal['Autosize'] | None = Field(
        default=None, json_schema_extra={'units': 'm3/s'}
    )
    minimum_flow_rate: float | Literal['Autosize'] | None = Field(
        default=None, json_schema_extra={'units': 'm3/s'}
    )
    fan_sizing_factor: float | None = Field(
        default=1.0,
        ge=1.0,
        json_schema_extra={'note': 'Applied to specified or autosized max fan airflow'},
    )
    fan_wheel_diameter: float = Field(
        ...,
        gt=0.0,
        json_schema_extra={
            'units': 'm',
    ...
    maximum_fan_static_efficiency: float = Field(
        ...,
        le=1.0,
        gt=0.0,
        json_schema_extra={
    ...
    euler_number_at_maximum_fan_static_efficiency: float = Field(
        ...,
        gt=0.0,
        json_schema_extra={
    ...
    maximum_dimensionless_fan_airflow: float = Field(
        ...,
        gt=0.0,
        json_schema_extra={
    ...
    motor_fan_pulley_ratio: float | Literal['', 'Autosize'] | None = Field(
        default=1.0,
        json_schema_extra={
            'note': 'Ratio of motor pulley diameter to fan pulley diameter'
        },
    )
    belt_maximum_torque: float | Literal['Autosize'] = Field(
        ...,
        json_schema_extra={
            'units': 'N-m',
    ...
    belt_sizing_factor: float | None = Field(
        default=1.0,
        ge=1.0,
        json_schema_extra={
    ...
    belt_fractional_torque_transition: float | None = Field(
        default=0.167,
        ge=0.0,
        le=1.0,
        json_schema_extra={
    ...
    motor_maximum_speed: float = Field(
        ...,
        gt=0.0,
        json_schema_extra={
            'units': 'rev/min',
    ...
    maximum_motor_output_power: float | Literal['Autosize'] = Field(
        ...,
        json_schema_extra={
            'units': 'W',
    ...
    motor_sizing_factor: float | None = Field(
        default=1.0,
        ge=1.0,
        json_schema_extra={
    ...
    motor_in_airstream_fraction: float | None = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        json_schema_extra={
    ...
    vfd_efficiency_type: Literal['Power', 'Speed'] | None = Field(
        default=None,
        json_schema_extra={
    ...
    maximum_vfd_output_power: float | Literal['Autosize'] = Field(
        ...,
        json_schema_extra={'units': 'W', 'note': 'Maximum power input to motor by VFD'},
    )
    vfd_sizing_factor: float | None = Field(
        default=1.0,
        ge=1.0,
        json_schema_extra={
...
class FanSystemModel(IDFBaseModel):
    ...
    air_inlet_node_name: str = Field(...)
    air_outlet_node_name: str = Field(...)
    design_maximum_air_flow_rate: float | Literal['Autosize'] = Field(
        ..., json_schema_extra={'units': 'm3/s'}
    )
    speed_control_method: Literal['', 'Continuous', 'Discrete'] | None = Field(
        default='Discrete'
    )
    electric_power_minimum_flow_rate_fraction: float | None = Field(
        default=0.2, ge=0.0, le=1.0
    )
    design_pressure_rise: float = Field(..., gt=0.0, json_schema_extra={'units': 'Pa'})
    motor_efficiency: float | None = Field(default=0.9, le=1.0, gt=0.0)
    motor_in_air_stream_fraction: float | None = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        json_schema_extra={
    ...
    design_electric_power_consumption: float | Literal['', 'Autosize'] | None = Field(
        default='Autosize',
        json_schema_extra={
            'units': 'W',
    ...
    design_power_sizing_method: (
        ...
        | None
    ) = Field(default='PowerPerFlowPerPressure')
    electric_power_per_unit_flow_rate: float | None = Field(
        default=None, json_schema_extra={'units': 'W/(m3/s)'}
    )
    electric_power_per_unit_flow_rate_per_unit_pressure: float | None = Field(
    ...
    electric_power_function_of_flow_fraction_curve_name: (
        ...
        json_schema_extra={
            'object_list': ['UnivariateFunctions'],
            'note': 'independent variable is normalized flow rate, current flow divided by Design Maximum Air Flow Rate. dependent variable is modification factor multiplied by Design Power Consumption. This field is r...',
        },
    )
    night_ventilation_mode_pressure_rise: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'Pa',
    ...
    night_ventilation_mode_flow_fraction: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
        json_schema_extra={
    ...
    motor_loss_radiative_fraction: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
        json_schema_extra={
    ...
    number_of_speeds: int | None = Field(
        default=1,
        json_schema_extra={
            'note': 'number of different speed levels available when Speed Control Method is set to Discrete Speed need to be arranged in increasing order in remaining field sets. If set to 1, or omitted, and Speed Con...'
        },
    )
...
class FanVariableVolume(IDFBaseModel):
    ...
    fan_power_minimum_flow_rate_input_method: (
        Literal['', 'FixedFlowRate', 'Fraction'] | None
    ) = Field(default='Fraction')
    fan_power_minimum_flow_fraction: float | None = Field(default=0.25, ge=0.0, le=1.0)
    fan_power_minimum_air_flow_rate: float | None = Field(
    ...
    fan_power_coefficient_1: float | None = Field(
        default=None,
        json_schema_extra={
            'note': 'all Fan Power Coefficients should not be 0.0 or no fan power will be consumed. Fan Power Coefficients are specified as function of full flow rate/power Equation:'
        },
    )
...
class FanZoneExhaust(IDFBaseModel):
    ...
    system_availability_manager_coupling_mode: (
        Literal['', 'Coupled', 'Decoupled'] | None
    ) = Field(
        default='Coupled',
        json_schema_extra={
