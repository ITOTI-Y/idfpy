...
class WaterUseConnections(IDFBaseModel):
    ...
    drain_water_heat_exchanger_type: (
        Literal['', 'CounterFlow', 'CrossFlow', 'Ideal', 'None'] | None
    ) = Field(default='None')
    drain_water_heat_exchanger_destination: (
        Literal['', 'Equipment', 'Plant', 'PlantAndEquipment'] | None
    ) = Field(default='Plant')
    drain_water_heat_exchanger_u_factor_times_area: float | None = Field(
...
class WaterUseRainCollector(IDFBaseModel):
    ...
    collection_loss_factor: float | None = Field(
        default=None,
        json_schema_extra={
...
class WaterUseStorage(IDFBaseModel):
    ...
    maximum_capacity: float | None = Field(
        default=None,
        json_schema_extra={'units': 'm3', 'note': 'Defaults to unlimited capacity.'},
    )
    initial_volume: float | None = Field(
        default=None, json_schema_extra={'units': 'm3'}
    )
    design_in_flow_rate: float | None = Field(
        default=None,
        json_schema_extra={'units': 'm3/s', 'note': 'Defaults to unlimited flow.'},
    )
    design_out_flow_rate: float | None = Field(
        default=None,
        json_schema_extra={'units': 'm3/s', 'note': 'Defaults to unlimited flow.'},
    )
    ...
    type_of_supply_controlled_by_float_valve: (
        Literal['GroundwaterWell', 'Mains', 'None', 'OtherTank'] | None
    ) = Field(default=None)
    float_valve_on_capacity: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'm3',
    ...
    float_valve_off_capacity: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'm3',
    ...
    backup_mains_capacity: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'm3',
...
class WaterUseWell(IDFBaseModel):
    ...
    pump_rated_flow_rate: float | None = Field(
        default=None, json_schema_extra={'units': 'm3/s'}
    )
    pump_rated_head: float | None = Field(
        default=None, json_schema_extra={'units': 'Pa'}
    )
    pump_rated_power_consumption: float | None = Field(
        default=None, json_schema_extra={'units': 'W'}
    )
    pump_efficiency: float | None = Field(default=None)
    well_recovery_rate: float | None = Field(
        default=None, json_schema_extra={'units': 'm3/s'}
    )
    nominal_well_storage_volume: float | None = Field(
        default=None, json_schema_extra={'units': 'm3'}
    )
    water_table_depth_mode: Literal['Constant', 'Scheduled'] | None = Field(
        default=None
    )
