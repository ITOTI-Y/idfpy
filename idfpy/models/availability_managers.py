...
class AvailabilityManagerAssignmentListManagersItem(IDFBaseModel):
    ...
    availability_manager_object_type: Literal[
...
class AvailabilityManagerDifferentialThermostat(IDFBaseModel):
    ...
    temperature_difference_on_limit: float = Field(
        ..., json_schema_extra={'units': 'deltaC'}
    )
    temperature_difference_off_limit: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'deltaC',
...
class AvailabilityManagerHybridVentilation(IDFBaseModel):
    ...
    ventilation_control_mode_schedule_name: ScheduleNamesRef = Field(
        ...
        json_schema_extra={
            ...
            'note': 'The Ventilation control mode contains appropriate integer control types. 0 - uncontrolled (Natural ventilation and HVAC system are controlled by themselves) 1 = Temperature control 2 = Enthalpy con...',
        },
    )
    ...
    minimum_outdoor_enthalpy: float | None = Field(
        default=None,
        gt=0.0,
        lt=300000.0,
        json_schema_extra={
    ...
    maximum_outdoor_enthalpy: float | None = Field(
        default=None,
        gt=0.0,
        lt=300000.0,
        json_schema_extra={
    ...
    minimum_outdoor_dewpoint: float | None = Field(
        default=-100.0,
        ge=-100.0,
        le=100.0,
        json_schema_extra={
    ...
    maximum_outdoor_dewpoint: float | None = Field(
        default=100.0,
        ge=-100.0,
        le=100.0,
        json_schema_extra={
    ...
    zoneventilation_object_name: VentilationNamesRef | None = Field(
        ...
        json_schema_extra={
            ...
            'note': 'This field has not been instrumented to work with global Zone or Zone List names option for Ventilation:DesignFlowRate. In order to use, you must enter the single <Ventilation:DesignFlowRate> name ...',
        },
    )
    minimum_hvac_operation_time: float | None = Field(
        default=0.0,
        ge=0.0,
        json_schema_extra={
            'units': 'minutes',
    ...
    minimum_ventilation_time: float | None = Field(
        default=0.0,
        ge=0.0,
        json_schema_extra={
            'units': 'minutes',
...
class AvailabilityManagerNightCycle(IDFBaseModel):
    ...
    thermostat_tolerance: float | None = Field(
        default=1.0, json_schema_extra={'units': 'deltaC'}
    )
    cycling_run_time_control_type: (
        Literal['', 'FixedRunTime', 'Thermostat', 'ThermostatWithMinimumRunTime'] | None
    ) = Field(default='FixedRunTime')
...
class AvailabilityManagerNightVentilation(IDFBaseModel):
    ...
    ventilation_temperature_difference: float | None = Field(
        default=2.0,
        json_schema_extra={
            'units': 'deltaC',
    ...
    ventilation_temperature_low_limit: float | None = Field(
        default=15.0,
        json_schema_extra={
            'units': 'C',
    ...
    night_venting_flow_fraction: float | None = Field(
        default=1.0,
        ge=0.0,
        json_schema_extra={
...
class AvailabilityManagerOptimumStart(IDFBaseModel):
    ...
    control_algorithm: (
        ...
        | None
    ) = Field(default='AdaptiveASHRAE')
    constant_temperature_gradient_during_cooling: float | None = Field(
        default=None, json_schema_extra={'units': 'deltaC/hr'}
    )
    constant_temperature_gradient_during_heating: float | None = Field(
        default=None, json_schema_extra={'units': 'deltaC/hr'}
    )
    initial_temperature_gradient_during_cooling: float | None = Field(
        default=None, json_schema_extra={'units': 'deltaC/hr'}
    )
    initial_temperature_gradient_during_heating: float | None = Field(
        default=None, json_schema_extra={'units': 'deltaC/hr'}
    )
    constant_start_time: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'hr',
    ...
    number_of_previous_days: int | None = Field(
        default=2,
        ge=2,
        le=5,
        json_schema_extra={
