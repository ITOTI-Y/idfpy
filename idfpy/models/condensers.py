...
if TYPE_CHECKING:
    from .location import SiteGroundTemperatureUndisturbedFiniteDifference, SiteGroundTemperatureUndisturbedKusudaAchenbach, SiteGroundTemperatureUndisturbedXing, SizingPeriodWeatherFileDays
...
and air/water flow rate inputs are for the entire tower."""
    ...
    @property
    def model_coefficient(self) -> CoolingTowerPerformanceCoolTools | CoolingTowerPerformanceYorkCalc | None:
    ...
    @property
    def fan_power_ratio_function_of_air_flow_rate_ratio_curve(self) -> IDFBaseModel | None:
...
the capacity and air/water flow rate inputs are for the entire tower."""
    ...
    @property
    def fan_power_modifier_function_of_air_flow_rate_ratio_curve(self) -> IDFBaseModel | None:
    ...
    @property
    def u_factor_times_area_modifier_function_of_air_flow_ratio_curve(self) -> IDFBaseModel | None:
    ...
    @property
    def u_factor_times_area_modifier_function_of_wetbulb_temperature_difference_curve(self) -> IDFBaseModel | None:
    ...
    @property
    def u_factor_times_area_modifier_function_of_water_flow_ratio_curve(self) -> IDFBaseModel | None:
...
as a counter flow heat exchanger."""
    ...
    design_entering_water_temperature: float | Literal['', 'Autosize'] | None = Field(default='Autosize', json_schema_extra={'units': 'C', 'note': 'Only used for Performance Input Method = UserSpecifiedDesignCapacity; for other Performance Input Methods, this field is ignored. Design Entering Water Temperature must be greater than Design Enter...'})
    design_entering_air_temperature: float | None = Field(default=35.0, gt=0.0, json_schema_extra={'units': 'C', 'note': 'Only used for Performance Input Method = UserSpecifiedDesignCapacity; for other Performance Input Methods, this field is ignored. Design Entering Air Temperature must be greater than Design Enterin...'})
    design_entering_air_wet_bulb_temperature: float | None = Field(default=25.6, gt=0.0, json_schema_extra={'units': 'C', 'note': 'Only used for Performance Input Method = UserSpecifiedDesignCapacity; for other Performance Input Methods, this field is ignored. Design Entering Air Wet-bulb Temperature must be less than Design E...'})
    capacity_control: Literal['', 'FanCycling', 'FluidBypass'] | None = Field(default='FanCycling')
    sizing_factor: float | None = Field(default=1.0, gt=0.0, json_schema_extra={'note': 'Multiplies the autosized capacity and flow rates'})
    evaporation_loss_mode: Literal['', 'LossFactor', 'SaturatedExit'] | None = Field(default='SaturatedExit')
...
\"g-functions\""""
    ...
    @property
    def ghe_vertical_properties_object(self) -> GroundHeatExchangerVerticalProperties | None:
...
specified."""
    ...
    g_function_calculation_method: Literal['', 'FullDesign', 'UBHWTcalc', 'UHFcalc'] | None = Field(default='UHFcalc')
    ghe_vertical_sizing_object_type: Literal['GroundHeatExchanger:Vertical:Sizing:Rectangle'] | None = Field(default=None)
    ghe_vertical_sizing_object_name: GroundHeatExchangerVerticalSizingNamesRef | None = Field(default=None, json_schema_extra={'object_list': ['GroundHeatExchangerVerticalSizingNames'], 'note': 'Only needed when g-function calculation method is FullDesign'})
    ...
    @property
    def ghe_vertical_responsefactors_object(self) -> GroundHeatExchangerResponseFactors | None:
        ...
        if idf is None:
            raise RuntimeError("Not bound to IDF")
        return idf._resolve_forward(v, ['GroundHeatExchangerVerticalResponseFactorNames'])

    @property
    def ghe_vertical_sizing_object(self) -> GroundHeatExchangerVerticalSizingRectangle | None:
...
design and sizing."""
    ...
    _provider_fields: ClassVar[frozenset[str]] = frozenset({"name"})
    name: str = Field(...)
    sizingperiod_weatherfiledays_name: SizingPeriodWeatherFileDaysRef = Field(..., json_schema_extra={'object_list': ['SizingPeriodWeatherFileDays'], 'note': 'Specifies the SizingPeriod:WeatherFileDays required to perform GHE sizing'})
    design_flow_rate_per_borehole: float | None = Field(default=0.0005, gt=0.0, json_schema_extra={'units': 'm3/s', 'note': 'Design flow rate per borehole used to size borehole field'})
    available_borehole_field_length: float = Field(..., gt=0.0, json_schema_extra={'units': 'm', 'note': 'Available length of the area available for designing the borehole field'})
    available_borehole_field_width: float = Field(..., gt=0.0, json_schema_extra={'units': 'm', 'note': 'Available width of the area available for designing the borehole field'})
    maximum_number_of_boreholes: float = Field(..., gt=0.0, json_schema_extra={'note': 'User specified maximum number of boreholes'})
    minimum_borehole_spacing: float | None = Field(default=4.0, gt=0.0, json_schema_extra={'units': 'm', 'note': 'Minimum borehole-to-borehole spacing'})
    maximum_borehole_spacing: float | None = Field(default=6.0, gt=0.0, json_schema_extra={'units': 'm', 'note': 'Maximum borehole-to-borehole spacing'})
    minimum_borehole_vertical_length: float | None = Field(default=60.0, gt=0.0, json_schema_extra={'units': 'm', 'note': 'Minimum drilled depth of the vertical borehole'})
    maximum_borehole_vertical_length: float | None = Field(default=135.0, gt=0.0, json_schema_extra={'units': 'm', 'note': 'Maximum drilled depth of the vertical borehole'})
    minimum_exiting_fluid_temperature_for_sizing: float | None = Field(default=5.0, gt=0.0, json_schema_extra={'units': 'C', 'note': 'Minimum ground heat exchanger exiting fluid temperature used when designing the borehole field'})
    maximum_exiting_fluid_temperature_for_sizing: float | None = Field(default=35.0, gt=0.0, json_schema_extra={'units': 'C', 'note': 'Maximum ground heat exchanger exiting fluid temperature used when designing the borehole field'})
