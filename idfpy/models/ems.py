...
class EnergyManagementSystemProgramCallingManagerProgramsItem(IDFBaseModel):
    ...
    @property
    def program(self) -> EnergyManagementSystemProgram | EnergyManagementSystemSubroutine | None:
        v = self.program_name
        if not v:
            return None
        idf = self._idf
...
class EnergyManagementSystemActuator(IDFBaseModel):
    ...
    name: str = Field(..., json_schema_extra={'note': 'This name becomes a variable for use in Erl programs no spaces or other special characters (minus, plus, slashes, etc) allowed in name'})
...
class EnergyManagementSystemCurveOrTableIndexVariable(IDFBaseModel):
    ...
    def curve_or_table_object(self) -> IDFBaseModel | None:
        ...
        if idf is None:
            raise RuntimeError("Not bound to IDF")
        return idf._resolve_forward(v, ['BivariateFunctions', 'MultivariateFunctions', 'QuadvariateFunctions', 'QuintvariateFunctions', 'TrivariateFunctions', 'UnivariateFunctions'])
