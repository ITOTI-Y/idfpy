...
class Building(IDFBaseModel):
    ...
    north_axis: float | None = Field(
        default=0.0,
        json_schema_extra={'units': 'deg', 'note': 'degrees from true North'},
    )
    terrain: Literal['', 'City', 'Country', 'Ocean', 'Suburbs', 'Urban'] | None = Field(
        default='Suburbs',
        json_schema_extra={
    ...
    loads_convergence_tolerance_value: float | None = Field(
        default=0.04,
        le=0.5,
        gt=0.0,
        json_schema_extra={
    ...
    temperature_convergence_tolerance_value: float | None = Field(
        default=0.4, le=0.5, gt=0.0, json_schema_extra={'units': 'deltaC'}
    )
    solar_distribution: (
        Literal[
            '',
    ...
    maximum_number_of_warmup_days: int | None = Field(
        default=25,
        gt=0,
        json_schema_extra={
    ...
    minimum_number_of_warmup_days: int | None = Field(
        default=1,
        gt=0,
        json_schema_extra={
...
class ConvergenceLimits(IDFBaseModel):
    ...
    minimum_system_timestep: int | None = Field(
        default=None,
        ge=0,
        le=60,
        json_schema_extra={
    ...
    minimum_plant_iterations: int | None = Field(
        default=2,
        ge=1,
        json_schema_extra={
            'note': 'Controls the minimum number of plant system solver iterations within a single HVAC iteration Larger values will increase runtime but might improve solution accuracy for complicated plant systems Co...'
        },
    )
    maximum_plant_iterations: int | None = Field(
        default=8,
        ge=2,
        json_schema_extra={
...
class HVACSystemRootFindingAlgorithm(IDFBaseModel):
    ...
    algorithm: (
        ...
        | None
    ) = Field(default='RegulaFalsi')
    number_of_iterations_before_algorithm_switch: int | None = Field(
        default=5,
        json_schema_extra={
...
class HeatBalanceAlgorithm(IDFBaseModel):
    ...
    surface_temperature_upper_limit: float | None = Field(
        default=200.0, ge=200.0, json_schema_extra={'units': 'C'}
    )
    minimum_surface_convection_heat_transfer_coefficient_value: float | None = Field(
        default=0.1, gt=0.0, json_schema_extra={'units': 'W/m2-K'}
    )
    maximum_surface_convection_heat_transfer_coefficient_value: float | None = Field(
        default=1000.0, ge=1.0, json_schema_extra={'units': 'W/m2-K'}
    )
...
class HeatBalanceSettingsConductionFiniteDifference(IDFBaseModel):
    ...
    difference_scheme: (
        Literal['', 'CrankNicholsonSecondOrder', 'FullyImplicitFirstOrder'] | None
    ) = Field(default='FullyImplicitFirstOrder')
    space_discretization_constant: float | None = Field(
        default=3.0, json_schema_extra={'note': 'increase or decrease number of nodes'}
    )
    relaxation_factor: float | None = Field(default=1.0, ge=0.01, le=1.0)
    inside_face_surface_temperature_convergence_criteria: float | None = Field(
        default=0.002, ge=1e-07, le=0.01
    )
...
class PerformancePrecisionTradeoffs(IDFBaseModel):
    ...
    use_coil_direct_solutions: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
        json_schema_extra={
    ...
    maxzonetempdiff: float | None = Field(
        default=0.3,
        ge=0.1,
        le=3.0,
        json_schema_extra={
    ...
    maxalloweddeltemp: float | None = Field(
        default=0.002,
        ge=0.002,
        le=0.1,
        json_schema_extra={
...
class ShadowCalculation(IDFBaseModel):
    ...
    shading_calculation_method: (
        Literal['', 'Imported', 'PixelCounting', 'PolygonClipping', 'Scheduled'] | None
    ) = Field(
        default='PolygonClipping',
        json_schema_extra={
            'note': 'Select between CPU-based polygon clipping method, the GPU-based pixel counting method, or importing from external shading data. If PixelCounting is selected and GPU hardware (or GPU emulation) is n...'
        },
    )
    shading_calculation_update_frequency_method: (
        Literal['', 'Periodic', 'Timestep'] | None
    ) = Field(
        default='Periodic',
        json_schema_extra={
    ...
    shading_calculation_update_frequency: int | None = Field(
        default=20,
        ge=1,
        json_schema_extra={
    ...
    maximum_figures_in_shadow_overlap_calculations: int | None = Field(
        default=15000,
        ge=200,
        json_schema_extra={
    ...
    pixel_counting_resolution: int | None = Field(
        default=512,
        json_schema_extra={
    ...
    sky_diffuse_modeling_algorithm: (
        Literal['', 'DetailedSkyDiffuseModeling', 'SimpleSkyDiffuseModeling'] | None
    ) = Field(
    ...
    disable_self_shading_within_shading_zone_groups: Literal['', 'No', 'Yes'] | None = (
        Field(
            default='No',
            json_schema_extra={
                'note': 'If Yes, self-shading will be disabled from all exterior surfaces in a given Shading Zone Group to surfaces within the same Shading Zone Group. If both Disable Self-Shading Within Shading Zone Group...'
            },
        )
    )
    disable_self_shading_from_shading_zone_groups_to_other_zones: (
        ...
        default='No',
        json_schema_extra={
            'note': 'If Yes, self-shading will be disabled from all exterior surfaces in a given Shading Zone Group to all other zones in the model. If both Disable Self-Shading Within Shading Zone Groups and Disable S...'
        },
    )
    shading_zone_groups: list[ShadowCalculationShadingZoneGroupsItem] | None = Field(
        default=None
    )
...
class SimulationControl(IDFBaseModel):
    ...
    do_zone_sizing_calculation: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
        json_schema_extra={
            'note': 'If Yes, Zone sizing is accomplished from corresponding Sizing:Zone objects and autosize fields.'
        },
    )
    do_system_sizing_calculation: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
        json_schema_extra={
    ...
    do_hvac_sizing_simulation_for_sizing_periods: Literal['', 'No', 'Yes'] | None = (
        Field(
            default='No',
            json_schema_extra={
                'note': 'If Yes, SizingPeriod:* objects are executed additional times for advanced sizing. Currently limited to use with coincident plant sizing, see Sizing:Plant object'
            },
        )
    )
    maximum_number_of_hvac_sizing_simulation_passes: int | None = Field(
        ...
        ge=1,
        json_schema_extra={
            'note': 'the entire set of SizingPeriod:* objects may be repeated to fine tune size results this input sets a limit on the number of passes that the sizing algorithms can repeat the set'
        },
    )
...
class SurfaceConvectionAlgorithmOutside(IDFBaseModel):
    ...
    algorithm: (
        ...
        default='DOE-2',
        json_schema_extra={
            'note': 'SimpleCombined = Combined radiation and convection coefficient using simple ASHRAE model TARP = correlation from models developed by ASHRAE, Walton, and Sparrow et. al. MoWiTT = correlation from me...'
        },
    )
...
class Timestep(IDFBaseModel):
    ...
    number_of_timesteps_per_hour: int | None = Field(
        default=6,
        ge=1,
        le=60,
        json_schema_extra={
            'note': 'Number in hour: normal validity 4 to 60: 6 suggested Must be evenly divisible into 60 Allowable values include 1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30, and 60 Normal 6 is minimum as lower values may c...'
        },
    )
...
class ZoneAirContaminantBalance(IDFBaseModel):
    ...
    carbon_dioxide_concentration: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
    ...
    generic_contaminant_concentration: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
        json_schema_extra={
...
class ZoneAirHeatBalanceAlgorithm(IDFBaseModel):
    ...
    algorithm: (
        Literal['', 'AnalyticalSolution', 'EulerMethod', 'ThirdOrderBackwardDifference']
        | None
    ) = Field(
        default='ThirdOrderBackwardDifference',
        json_schema_extra={
            'note': 'Determines which algorithm will be used to solve the air heat balance.'
        },
    )
    do_space_heat_balance_for_sizing: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
        json_schema_extra={
            'note': 'If yes, space heat balance will be calculated and reported during sizing.'
        },
    )
    do_space_heat_balance_for_simulation: Literal['', 'No', 'Yes'] | None = Field(
        default='No',
        json_schema_extra={
            'note': 'If yes, space heat balance will be calculated and reported during simulation.'
        },
    )
...
class ZoneAirMassFlowConservation(IDFBaseModel):
    ...
    adjust_zone_mixing_and_return_for_air_mass_flow_balance: (
        ...
        default='None',
        json_schema_extra={
            'note': 'If "AdjustMixingOnly", zone mixing object flow rates are adjusted to balance the zone air mass flow and zone infiltration air flow may be increased or decreased if required in order to balance the ...'
        },
    )
    infiltration_balancing_method: (
        Literal['', 'AddInfiltrationFlow', 'AdjustInfiltrationFlow', 'None'] | None
    ) = Field(
        default='AddInfiltrationFlow',
        json_schema_extra={
            'note': 'This input field allows user to choose how zone infiltration flow is treated during the zone air mass flow balance calculation. AddInfiltrationFlow may add infiltration to the base flow specified i...'
        },
    )
    infiltration_balancing_zones: (
        Literal['', 'AllZones', 'MixingSourceZonesOnly'] | None
    ) = Field(
        default='MixingSourceZonesOnly',
        json_schema_extra={
            'note': 'This input field allows user to choose which zones are included in infiltration balancing. MixingSourceZonesOnly allows infiltration balancing only in zones which as source zones for mixing which a...'
        },
    )
...
class ZoneCapacitanceMultiplierResearchSpecial(IDFBaseModel):
    ...
    temperature_capacity_multiplier: float | None = Field(
        default=1.0,
        gt=0.0,
        json_schema_extra={
    ...
    humidity_capacity_multiplier: float | None = Field(
        default=1.0,
        gt=0.0,
        json_schema_extra={
    ...
    carbon_dioxide_capacity_multiplier: float | None = Field(
        default=1.0,
        gt=0.0,
        json_schema_extra={
    ...
    generic_contaminant_capacity_multiplier: float | None = Field(
        default=1.0,
        gt=0.0,
        json_schema_extra={
