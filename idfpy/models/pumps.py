...
class HeaderedPumpsConstantSpeed(IDFBaseModel):
    ...
    total_design_flow_rate: float | Literal['Autosize'] | None = Field(
        default=None,
        json_schema_extra={
            'units': 'm3/s',
    ...
    flow_sequencing_control_scheme: Literal['', 'Sequential'] | None = Field(
        default='Sequential'
    )
    design_pump_head: float | None = Field(
        default=179352.0,
        json_schema_extra={'units': 'Pa', 'note': 'default head is 60 feet'},
    )
    design_power_consumption: float | Literal['Autosize'] | None = Field(
        default=None,
        json_schema_extra={
            'units': 'W',
            'note': 'If the field is not autosized set to the power consumed by the pump bank when all the pumps are running at nominal flow When autosized the type of scaling factor is chosen in the input field Design...',
        },
    )
    motor_efficiency: float | None = Field(
        default=0.9,
        le=1.0,
        gt=0.0,
        json_schema_extra={
            'note': 'This is the motor efficiency only. When the Design Power Consumption is autosized using PowerPerFlowPerPressure, the Design Shaft Power per Unit Flow Rate per Unit Head is used in addition to the m...'
        },
    )
    ...
    zone_name: ZoneNamesRef | None = Field(
        ...
        json_schema_extra={
            ...
            'note': 'optional, if used pump losses transferred to zone as internal gains',
        },
    )
    skin_loss_radiative_fraction: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
        json_schema_extra={
    ...
    design_power_sizing_method: (
        Literal['', 'PowerPerFlow', 'PowerPerFlowPerPressure'] | None
    ) = Field(
        default='PowerPerFlowPerPressure',
        json_schema_extra={
            'note': 'Used to indicate which sizing factor is used to calculate Design Power Consumption. PowerPerFlow indicates that Design Electric Power per Unit Flow Rate is used as scaling factor. Design Power Cons...'
        },
    )
    design_electric_power_per_unit_flow_rate: float | None = Field(
        default=348701.1,
        gt=0.0,
        json_schema_extra={
            'units': 'W/(m3/s)',
    ...
    design_shaft_power_per_unit_flow_rate_per_unit_head: float | None = Field(
        default=1.282051282,
        gt=0.0,
        json_schema_extra={
            'units': 'W/((m3/s)-Pa)',
...
class HeaderedPumpsVariableSpeed(IDFBaseModel):
    ...
    minimum_flow_rate_fraction: float | None = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        json_schema_extra={
...
class PumpConstantSpeed(IDFBaseModel):
    ...
    pump_curve_name: UnivariateFunctionsRef | None = Field(
        ...
        json_schema_extra={
            ...
            'note': 'This references any single independent variable polynomial curve in order to do pressure vs. flow calculations for this pump. The available types are then: Linear, Quadratic, Cubic, and Quartic The...',
        },
    )
    impeller_diameter: float | None = Field(
        default=None,
        json_schema_extra={'units': 'm', 'note': '"D" in above expression in field A6'},
    )
    rotational_speed: float | None = Field(
        default=None,
        json_schema_extra={
            'units': 'rev/min',
...
class PumpVariableSpeed(IDFBaseModel):
    ...
    inlet_node_name: str = Field(...)
    outlet_node_name: str = Field(...)
    design_maximum_flow_rate: float | Literal['Autosize'] | None = Field(
        default=None, json_schema_extra={'units': 'm3/s'}
    )
    design_pump_head: float | None = Field(
        default=179352.0,
    ...
    coefficient_3_of_the_part_load_performance_curve: float | None = Field(default=0.0)
    coefficient_4_of_the_part_load_performance_curve: float | None = Field(default=0.0)
    design_minimum_flow_rate: float | Literal['', 'Autosize'] | None = Field(
        default='Autosize',
        json_schema_extra={
            'units': 'm3/s',
    ...
    design_minimum_flow_rate_fraction: float | None = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        json_schema_extra={'note': 'Used to size Design Minimum Flow Rate'},
    )
    end_use_subcategory: str | None = Field(
        default='General',
        json_schema_extra={
...
class PumpVariableSpeedCondensate(IDFBaseModel):
    ...
    inlet_node_name: str = Field(...)
    outlet_node_name: str = Field(...)
    design_steam_volume_flow_rate: float | Literal['Autosize'] | None = Field(
        default=None,
        json_schema_extra={
            'units': 'm3/s',
