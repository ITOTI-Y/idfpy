...
class DaylightingControlsControlDataItem(IDFBaseModel):
    ...
    illuminance_setpoint_at_reference_point: float | None = Field(
...
class DaylightingControls(IDFBaseModel):
    ...
    lighting_control_type: (
        Literal['', 'Continuous', 'ContinuousOff', 'Stepped'] | None
    ) = Field(default='Continuous')
    minimum_input_power_fraction_for_continuous_or_continuousoff_dimming_control: (
        float | None
    ) = Field(default=0.3, ge=0.0, le=0.6)
    minimum_light_output_fraction_for_continuous_or_continuousoff_dimming_control: (
        float | None
    ) = Field(default=0.2, ge=0.0, le=0.6)
    number_of_stepped_control_steps: int | None = Field(
        default=1,
        ge=1,
        json_schema_extra={
    ...
    glare_calculation_azimuth_angle_of_view_direction_clockwise_from_zone_y_axis: (
        float | None
    ) = Field(default=0.0, ge=0.0, le=360.0, json_schema_extra={'units': 'deg'})
    maximum_allowable_discomfort_glare_index: float | None = Field(
        default=22.0,
        ge=1.0,
        json_schema_extra={'note': 'The default is for general office work'},
    )
    delight_gridding_resolution: float | None = Field(
        default=None,
        gt=0.0,
        json_schema_extra={
            'units': 'm2',
            'note': 'Maximum surface area for nodes in gridding all surfaces in the DElight zone. All reflective and transmitting surfaces will be subdivided into approximately square nodes that do not exceed this maxi...',
        },
    )
...
class DaylightingDELightComplexFenestration(IDFBaseModel):
    ...
    fenestration_rotation: float | None = Field(
        default=0.0,
        json_schema_extra={
            'units': 'deg',
            'note': 'In-plane counter-clockwise rotation angle of the Complex Fenestration optical reference direction and the base edge of the Complex Fenestration. The Rotation will typically be zero when the host an...',
        },
    )
...
class DaylightingDeviceLightWell(IDFBaseModel):
    ...
    perimeter_of_bottom_of_well: float = Field(
        ..., gt=0.0, json_schema_extra={'units': 'm'}
    )
    area_of_bottom_of_well: float = Field(
        ..., gt=0.0, json_schema_extra={'units': 'm2'}
    )
...
class DaylightingDeviceTubular(IDFBaseModel):
    ...
    effective_thermal_resistance: float | None = Field(
        default=0.28,
        gt=0.0,
        json_schema_extra={
            'units': 'm2-K/W',
    ...
    transition_lengths: list[DaylightingDeviceTubularTransitionLengthsItem] | None = (
        Field(default=None)
    )
...
class DaylightingReferencePoint(IDFBaseModel):
    ...
    y_coordinate_of_reference_point: float = Field(
        ..., json_schema_extra={'units': 'm'}
    )
...
class OutputIlluminanceMap(IDFBaseModel):
    ...
    x_minimum_coordinate: float | None = Field(
        default=0.0, json_schema_extra={'units': 'm'}
    )
    x_maximum_coordinate: float | None = Field(
        default=1.0, json_schema_extra={'units': 'm'}
    )
    number_of_x_grid_points: int | None = Field(
        default=2,
        ge=1,
        json_schema_extra={
    ...
    y_minimum_coordinate: float | None = Field(
        default=0.0, json_schema_extra={'units': 'm'}
    )
    y_maximum_coordinate: float | None = Field(
        default=1.0, json_schema_extra={'units': 'm'}
    )
    number_of_y_grid_points: int | None = Field(
        default=2,
        ge=1,
        json_schema_extra={
            'note': 'Maximum number of total grid points must be <= 2500 (X*Y)'
