...
class SpaceHVACZoneEquipmentMixerSpacesItem(IDFBaseModel):
    ...
    space_fraction: float | Literal['', 'Autosize'] | None = Field(
        ...
        json_schema_extra={
            'units': 'dimensionless',
            'note': 'Fraction of the Zone Equipment Inlet Node airflow drawn from this space.',
        },
    )
    space_node_name: str = Field(
        ...,
        json_schema_extra={
            'note': 'Matches a SpaceHVAC:EquipmentConnections Exhaust Node Name'
        },
    )
...
class SpaceHVACZoneEquipmentSplitterSpacesItem(IDFBaseModel):
    ...
    space_fraction: float | Literal['', 'Autosize'] | None = Field(
        ...
        json_schema_extra={
            'units': 'dimensionless',
            'note': 'Fraction of this zone equipment output or airflow distributed to this space.',
        },
    )
    space_supply_node_name: str | None = Field(
        default=None,
        json_schema_extra={
            'note': 'Only used for airflow equipment Matches a SpaceHVAC:EquipmentConnections Inlet Node Name'
        },
    )
...
class ZoneHVACEquipmentListEquipmentItem(IDFBaseModel):
    ...
    zone_equipment_heating_or_no_load_sequence: int = Field(
        ...,
        ge=0,
        json_schema_extra={
...
class SpaceHVACEquipmentConnections(IDFBaseModel):
    ...
    space_return_air_node_1_flow_rate_basis_node_or_nodelist_name: str | None = Field(
        default=None,
        json_schema_extra={
...
class SpaceHVACZoneEquipmentMixer(IDFBaseModel):
    ...
    zone_equipment_inlet_node_name: str = Field(
        ...,
        json_schema_extra={
            'note': 'The inlet node from the zone equipment that will be mixed from the spaces. Must match a Zone Exhaust Node for this zone.'
        },
    )
    space_fraction_method: (
        ...
        json_schema_extra={
            'note': 'The basis used to autosize the space output fractions.'
        },
    )
...
class SpaceHVACZoneEquipmentSplitter(IDFBaseModel):
    ...
    zone_equipment_outlet_node_name: str | None = Field(
        default=None,
        json_schema_extra={
            'note': 'Only used for airflow equipment. The outlet node from the zone equipment that will be split to the spaces.'
        },
    )
    thermostat_control_method: Literal['', 'Ideal', 'Maximum', 'SingleSpace'] | None = (
        Field(
            default='SingleSpace',
            json_schema_extra={
                'note': 'SingleSpace satisfies the thermostat in the Control Space Name Maximum satisfies the thermostat in the connected spaces with the highest deviation from setpoint Ideal ignores the Space Output Fract...'
            },
        )
    )
    control_space_name: SpaceNamesRef | None = Field(
        ...
        json_schema_extra={
            ...
            'note': 'This field is only used when Thermostat Control Method = SingleSpace.',
        },
    )
    space_fraction_method: (
        Literal[
...
class SpaceHVACZoneReturnMixer(IDFBaseModel):
    ...
    zone_return_air_node_name: str = Field(
        ...,
        json_schema_extra={
            'note': 'The zone return air node will be mixed from the spaces. Must match a Zone Return Air Node for this zone.'
        },
    )
...
class ZoneHVACEquipmentList(IDFBaseModel):
    ...
    load_distribution_scheme: (
        Literal[
            '', 'SequentialLoad', 'SequentialUniformPLR', 'UniformLoad', 'UniformPLR'
        ]
        | None
